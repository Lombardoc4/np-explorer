...
import { ParkProps } from "../../pages/ParkContent/Park";
...
const sortActivities = (activeParks: any) => {
    ...
    const activities = activeParks.reduce((acc: any, park: any) => {
        ...
        park.activities.forEach((activity: any) => {
            // Loop 2
            const existingItem = acc.find((obj: { name: string; count: number }) => obj.name === activity.name); // Loop 3
...
export const ParkCardFilters = ({ otherParks, toggleFilter }: ParkCardFiltersProps) => {
    ...
    const dropdownOptions =
        otherParks.length > 0
            ? otherParks.map((park: any) => ({ value: park.parkCode, title: park.fullName }))
            : noParks;
        ...
        <div className='filters'>
            ...
            <FilterCard ref={filterRef} className='container' style={{ opacity: showFilters ? 1 : 0, pointerEvents: showFilters ? 'all' : 'none' }}>
                <FilterCell>
                    ...
                    <div className='checkbox'>
                        <input onClick={handleInput} type='radio' id='free' name='entranceFees' value='free' />
                        <label htmlFor='free'>Free</label>
                    </div>
                    <div className='checkbox'>
                        <input onClick={handleInput} type='radio' id='paid' name='entranceFees' value='paid' />
                ...
                <FilterCell>
                    ...
                    {activities.map(({ name, count }: { name: string; count: number }) => (
                        <div key={name} className='checkbox'>
                            <input onClick={handleInput} type='checkbox' id={name} name='activities' value={name} />
                            <label htmlFor={name}>
                                {name} ({count})
                            </label>
                        </div>
