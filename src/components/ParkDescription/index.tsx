...
const FeeCard = ({cost, title, description} : {cost: string, title: string, description: string}) => {
    ...
    return (
    <StyledFeeCard $active={active}>
        <div className="price">
            <p>${cost}</p>
            <div className="info" ref={ref} onClick={() => setActive(!active)} >
                <p>{description}</p>
...
const FeeSection = ({entranceFees} : any) => {
    if (entranceFees.length === 0) return (
                ...
                <ContactInfo>
                    ...
                    { park.contacts.phoneNumbers.length > 0 &&
                        park.contacts.phoneNumbers.map(({phoneNumber}: {phoneNumber: string}) => (
                            <p key={phoneNumber}>Phone: <a href={`tel:${phoneNumber}`}>{phoneNumber.slice(0, 3)}-{phoneNumber.slice(3, 6)}-{phoneNumber.slice(6, 10)}</a></p>
                        ))   
                    }
