...
const getCoords = (state: StateProps) => {
  return USMapCords.features.find(
    (stateCoords) =>
      stateCoords.properties && state.name === stateCoords.properties.name,
  );
};
...
const CustomMap = ({ states, parkCoords }: IMap) => {
  ...
  const baseView =
    active.length === 1
      ? ([active[0].latitude, active[0].longitude] as LatLngTuple)
      : ([37.8, -96] as LatLngTuple);
  ...
  states = states.reduce((acc: StateProps[], s) => {
    if (!acc.find((obj) => obj.id === s.id)) acc.push(s);
    return acc;
  }, []);
