...
const reduceForecast = (periods: any[]) => {
  return periods.reduce((acc: any[], forecast: any) => {
    ...
    const name = todayAlts.includes(forecast.name)
      ? 'Today'
      : forecast.name.split(' ')[0]; // Extract the day name
...
export const WeatherDisplay = ({
  ...
  if (hourly.length <= 0 || sevenDay.length <= 0 || location.city.length <= 0)
    return (
      <div className='grid rounded-xl border bg-green-200 py-4 dark:bg-green-900 dark:text-white'>
        Loading Weather
      </div>
    );
        ...
        <div>
          <div className='mx-auto grid'>
            ...
            <p>
              ...
              {current.probabilityOfPrecipitation.value}% Percipitation
            </p>
            <p>
              <WeatherIcon id={'humidity'} /> {current.relativeHumidity.value}%
              Humidity
            </p>
            <p>
              <WeatherIcon id={'wind-' + current.windDirection} />{' '}
