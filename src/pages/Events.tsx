...
const EventSection = ({ event }: any) => {
  const date = new Date(event.date.replace(/-/g, '/')).toLocaleDateString(
    'en-US',
    {
      weekday: 'long',
      year: 'numeric',
  ...
  const times = event.times.map((t: any, i: number) => (
    <span key={'time' + i}>
      {i !== 0 && '|'}{' '}
      {t.timestart.indexOf(0) === 0 ? t.timestart.slice(1) : t.timestart}
    </span>
  ));
  ...
  const image = event.images.length > 0 && (
    <StyledCard className='img-container'>
      <img
        src={'https://www.nps.gov' + event.images[0].url}
        alt={event.images[0].altText}
      />
    </StyledCard>
  );
          ...
          <StyledContactCard>
            ...
            <CardItem>
              {event.isregresrequired === 'true' ? (
                event.regresinfo || (
                  <span className='bold'>Registration required</span>
                )
              ) : (
                <>
